...
var configDefaults = {
  ...
  autoFreeze: typeof process !== "undefined" ? undefined !== "production" :
  /* istanbul ignore next */
...
const getActionMap = (reducerModule, namespace) => Object.keys(reducerModule).reduce((actionMap, actionName) => ({ ...actionMap,
  [actionName]: namespace + NAME_SPACE_FLAG + actionName
}), {});

const getKey = str => str.substring(str.indexOf(NAME_SPACE_FLAG) + 1, str.length + 1);

const withReducerModule = ({
  state,
  action,
  reducer,
  namespace = ''
}) => Object.keys(reducer).map(key => namespace + NAME_SPACE_FLAG + key).includes(action.type) ? produce(state, draft => reducer[getKey(action.type)](draft, action)) : state;
...
const createModel = model => {
  ...
  const reducerModule = (state = model.state, action) => withReducerModule({
    state,
    action,
    reducer,
    namespace
...
const storeEnhancer = createStore => (reducer, preloadedState, enhancer) => {
  ...
  store.dispatch = (typeOrAction, payload) => typeof typeOrAction === 'object' ? oldDispatch(typeOrAction) : oldDispatch({
    type: typeOrAction,
    payload
  });
...
export { createModel, setActionToStore, storeEnhancer };
